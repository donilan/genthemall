...
<table class="list-table" >
	...
	<tr class="${s.count % 2 eq 0 ? 'even' : 'odd' } ${s.first? 'first': s.last ? 'last': ''}">
		...
		<td class="email editable" title="${vo.email}" >
			<span>${vo.email}</span>
			<input length="50" class="hide" type="text" name="email" value="${vo.email}" />
		</td>
		<td class="fax editable" title="${vo.fax}" >
			<span>${vo.fax}</span>
			<input length="20" class="hide" type="text" name="fax" value="${vo.fax}" />
		</td>
		...
		<td class="zipCode editable" title="${vo.zipCode}" >
			...
			<input length="10" class="hide" type="text" name="zipCode" value="${vo.zipCode}" />
		</td>
		<td class="contact editable" title="${vo.contact}" >
			<span>${vo.contact}</span>
			<input length="40" class="hide" type="text" name="contact" value="${vo.contact}" />
		</td>
		...
		<td class="deleted editable" title="${vo.deleted}" >
			...
			<input length="1" class="hide" type="text" name="deleted" value="${vo.deleted}" />
		</td>
		...
		<td class="areaId editable" title="${vo.areaId}" >
			<span>${vo.areaId}</span>
			<input length="11" class="hide" type="text" name="areaId" value="${vo.areaId}" />
		</td>
		...
		<td class="shopTypeId editable" title="${vo.shopTypeId}" >
			...
			<input length="11" class="hide" type="text" name="shopTypeId" value="${vo.shopTypeId}" />
		</td>
