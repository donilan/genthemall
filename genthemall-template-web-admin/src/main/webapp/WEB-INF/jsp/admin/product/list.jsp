...
<table class="list-table" >
	...
	<tr class="${s.count % 2 eq 0 ? 'even' : 'odd' } ${s.first? 'first': s.last ? 'last': ''}">
		...
		<td class="hnCode editable" title="${vo.hnCode}" >
			<span>${vo.hnCode}</span>
			<input class="hide" type="text" name="hnCode" value="${vo.hnCode}" />
		</td>
		<td class="unitId editable" title="${vo.unitId}" >
			<span>${vo.unitId}</span>
			<input class="hide" type="text" name="unitId" value="${vo.unitId}" />
		</td>
		<td class="shopId editable" title="${vo.shopId}" >
			<span>${vo.shopId}</span>
			<input class="hide" type="text" name="shopId" value="${vo.shopId}" />
		</td>
		<td class="originAreaId editable" title="${vo.originAreaId}" >
			<span>${vo.originAreaId}</span>
			<input class="hide" type="text" name="originAreaId" value="${vo.originAreaId}" />
		</td>
		<td class="productTypeId editable" title="${vo.productTypeId}" >
			<span>${vo.productTypeId}</span>
			<input class="hide" type="text" name="productTypeId" value="${vo.productTypeId}" />
		</td>
